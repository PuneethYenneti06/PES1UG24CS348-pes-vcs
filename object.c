...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
  // 1. Build the header
    ...
    header_len++; // Include the null terminator '\0' in the header length

    // 2. Prepare the full object (header + data) for hashing
    ...
    // 3. Compute hash and check if it already exists
